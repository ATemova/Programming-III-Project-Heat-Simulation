...
import java.util.concurrent.ExecutorService;  // Import for concurrent task execution
import java.util.concurrent.Executors;  // Import for creating thread pools
import java.util.concurrent.TimeUnit;  // Import for time unit constants
import java.util.concurrent.atomic.AtomicBoolean;  // Import for thread-safe boolean values
public class Server extends UnicastRemoteObject {
    // Maximum allowable temperature change for stability
    ...
    public Server(int width, int height, int points) throws RemoteException {
        ...
        initializeGrid(width, height, points); // Initialize the grid with base temperatures and heat sources
    }
    private void initializeGrid(int width, int height, int points) {
        ...
        // Set the base temperature for all cells in the grid
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                grid[i][j] = 20; // Base temperature
            }
        }
        // Place heat sources at random positions
    ...
    public void distributeWork() throws RemoteException {
        ...
        do {
            ...
            for (int chunk = 0; chunk < Runtime.getRuntime().availableProcessors(); chunk++) {
                final int from = chunk * chunkSize; // Starting index for this chunk
                final int to = (chunk == Runtime.getRuntime().availableProcessors() - 1) ? grid.length : from + chunkSize; // Ending index for this chunk
